...
def _calculate_correlation_at_target_lag(
    ...
    if param_hash in indicator_series_cache:
        ...
        # ----- CHANGE WARNING TO INFO -----
             ...
             logger.info(f"Cached indicator DF is empty for {param_hash[:8]}. Recomputing."); indicator_output_df = None; del indicator_series_cache[param_hash]
        # ----- END CHANGE -----
    ...
    if indicator_output_df is None or indicator_output_df.empty:
        # ----- CHANGE WARNING TO INFO -----
        # This remains an informational log about the outcome of computation
        logger.info(f"Indicator {'cache' if cache_hit else 'compute'} failed/empty for ID {config_id}.")
        # ----- END CHANGE -----
        return None
...
def _objective_function(params_list: List[Any], *, # Use keyword-only args after params_list
    ...
    if not parameter_generator.evaluate_conditions(full_params, indicator_def.get('conditions', [])):
        # Changed to DEBUG as this is expected during optimization exploration
    ...
    # Log less frequently (now INFO)
    if calls_done % 20 == 0 or calls_done == 1 or calls_done == calls_total: # Adjusted frequency
        logger.info(f"Progress ({indicator_name} L{target_lag}): Eval {calls_done}/{calls_total} ({percent:.1f}%) | Rate: {rate:.1f} evals/s | CacheHit: {'Yes' if cache_hit else 'No'} | Elapsed: {elapsed:.1f}s")
    ...
    if config_id is not None and pd.notna(correlation_value):
        try:
            ...
            # Export leaderboard text file if an update happened
            if updated:
                try:
    ...
    if pd.notna(correlation_value) and isinstance(correlation_value, (float, int, np.number)):
         ...
         log_id = config_id if config_id is not None else "N/A"
         # Changed to DEBUG as expected during exploration
    ...
    for target_lag in range(1, max_lag + 1):
        ...
        try:
             with warnings.catch_warnings():
                  ...
                  # Clip x0 to search space bounds
                  if x0:
                      try:
                          x0_clipped_vals = []
                          for i, dim in enumerate(search_space):
                              ...
                              eps = 1e-9
                              if not (low - eps <= val <= high + eps):
                                  # Changed to DEBUG
                                  logger.debug(f"Clipping x0 val {val} for dim '{dim.name}' (bounds: [{low}, {high}])")
                                  if isinstance(dim, Integer): clipped_val = max(low, min(high, int(np.round(val))))
                                  elif isinstance(dim, Real): clipped_val = max(low, min(high, float(val)))
                                  else: clipped_val = val # Categorical etc.
                                  x0_clipped_vals.append(clipped_val)
                              else:
                                  if isinstance(dim, Integer): val = int(np.round(val))
                                  elif isinstance(dim, Real): val = float(val)
                                  x0_clipped_vals.append(val)

                          x0_clipped = [x0_clipped_vals]
                          if x0_clipped != x0:
                              # Changed to DEBUG
                              logger.debug(f"Lag {target_lag}: Used clipped x0 initial point {x0_clipped} (original: {x0}).")
                          x0 = x0_clipped
                      except Exception as clip_err:
                           logger.error(f"Error clipping/validating x0 for lag {target_lag}: {clip_err}. Proceeding without x0.", exc_info=True)
                           x0, y0 = None, None

                  current_n_initial = min(n_initial_points_per_lag, n_calls_per_lag)
                  if x0 is not None and current_n_initial > 0:
                      current_n_initial = max(0, current_n_initial - 1)

                  result = gp_minimize(func=objective_partial, dimensions=search_space,
                                       acq_func=app_config.DEFAULTS["optimizer_acq_func"],
                                       n_calls=n_calls_per_lag, n_initial_points=current_n_initial,
                                       x0=x0, y0=y0, random_state=None, noise='gaussian')
             ...
             if best_objective_value < (1e6 - 1.0): # Found valid solution
                 ...
                 if best_config_info and 'config_id' in best_config_info:
                     ...
                     # This remains an ERROR as it indicates a logic flaw
                 ...
                 # Changed to INFO as this is a normal fallback
                 logger.info(f"Lag {target_lag}: Opt failed for {indicator_name}. Checking default.")
                 if default_config_id is not None and default_params_full:
                     ...
                     if default_corr is not None and pd.notna(default_corr):
                         ...
                         best_config_per_lag[target_lag] = {'params': default_params_full, 'config_id': default_config_id, 'correlation_at_lag': default_corr, 'score_at_lag': abs(default_corr)}
                          ...
                          # ----- CHANGE WARNING TO INFO -----
                          logger.info(f"Lag {target_lag}: Default ({default_config_id}) for {indicator_name} had NaN/None corr. No solution found for this lag.")
                          best_config_per_lag[target_lag] = None
                          # ----- END CHANGE -----
                 else:
                      # Keep as WARNING - default wasn't even available/valid
                      logger.warning(f"Lag {target_lag}: No valid default config available/evaluated for {indicator_name}. No solution found for this lag.")
                      best_config_per_lag[target_lag] = None
...
def _define_search_space(param_defs: Dict) -> Tuple[List, List[str], Dict, bool]:
    ...
    for name, details in sorted(param_defs.items()):
        ...
        is_num_tune = isinstance(p_type, type) and issubclass(p_type, (int, float)) and (default is not None or (min_j is not None and max_j is not None))
        ...
        elif default is not None: # Fallback: create small range around default
             b_src = "DefaultFallback"
             if issubclass(p_type, int):
                  min_v = max(1, default - 2)
                  max_v = default + 2
                  ...
                  min_v = max(2 if strict_min_2 else 1, min_v)
             elif issubclass(p_type, float):
                  step = max(0.01, abs(default * 0.1))
                  min_v = round(default - 2*step, 4)
                  max_v = round(default + 2*step, 4)
                  if name in factor_params: min_v = max(0.01, min_v)
                  elif name in dev_scalar_params: min_v = max(0.1, min_v)
                  elif name in period_params: min_v = max(2.0, min_v)
                  else: min_v = max(0.0, min_v)
             else: logger.warning(f"Param '{name}': Unexpected type '{p_type}' for default fallback. Fixing @ default."); fixed_params[name] = default; continue
        else: logger.warning(f"Param '{name}': No default/bounds. Skipping."); continue

        if min_v >= max_v - 1e-9:
             fix_val = default if default is not None else min_v
             if issubclass(p_type, int): fix_val = int(round(fix_val))
             logger.warning(f"Param '{name}': min ({min_v}) >= max ({max_v}) after fallback/JSON. Fixing @ {fix_val}.");
             fixed_params[name] = fix_val; continue

        low, high = min_v, max_v
        if issubclass(p_type, int):
            low_int, high_int = int(np.round(low)), int(np.round(high))
            if low_int >= high_int: high_int = low_int + 1; logger.warning(f"Adjusted Integer bounds for '{name}' due to rounding: [{low_int}, {high_int}]")
        ...
        if conn:
            try:
                if default_hash not in master_evaluated_configs:
                    ...
                    else: logger.error(f"Failed to create DB entry for default config: {defaults_full}");
