...
- `requirements.txt` is UTF-16 LE encoded. `uv` parses it fine, but `grep`/most tools treat it
  as binary; use `iconv -f UTF-16 -t UTF-8 requirements.txt` if you need to read/filter it.
