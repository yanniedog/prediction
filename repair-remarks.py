...
def process_file(path, name):
    try:
        ...
        mod_lines, found_correct_comment = [], False
        for line in lines:
            if line.strip() == correct_comment: found_correct_comment = True
            mod_lines.append(line)
        if not found_correct_comment: mod_lines.insert(0, f'{correct_comment}\n')
        with open(path, 'w', encoding='utf-8') as f: f.writelines(mod_lines)
        print(f"Modified {path}: {'Added correct comment.' if not found_correct_comment else 'No changes needed.'}")
