...
def fetch_available_indicators():
    return sorted([func for func in dir(compute_all_indicators) if not func.startswith("_")])

def generate_configurations(params, defaults):
    return [dict(zip(params, values)) for values in itertools.product(*[range(max(1, defaults[p] - 5), defaults[p] + 5) for p in params])]
