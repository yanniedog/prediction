...
def generate_statistical_summary(correlations:Dict[str,List[float]],max_lag:int)->pd.DataFrame:
    ...
    for col in correlations:
        best_count=sum(1 for lag in range(1,max_lag+1)if lag-1<len(correlations[col])and correlations[col][lag-1]==max(correlations[c][lag-1]if lag-1<len(correlations[c])else -np.inf for c in correlations))
        ...
        summary[col]={'Best Count':best_count,'Average Correlation':average_corr,'Max Correlation':max_corr,'Min Correlation':min_corr}
