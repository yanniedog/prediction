...
def perform_linear_regression(data,correlations,max_lag,time_interval,timestamp,base_csv_filename,future_datetime,lag_periods):
    ...
    lag_range=range(1,max_user_lag+1) if calc_all_lags else [max_user_lag]
    predictions_data=[]
    coefficients_data=[]
    ...
    for d in [predictions_dir,csv_dir,json_dir,plots_dir]:
    ...
    plt.fill_between(plot_dates,plot_lower_bounds,plot_upper_bounds,color='orange',alpha=0.3,label='95% CI')
    plt.xlabel('Date')
    plt.ylabel('Price')
...
def save_predictions_and_coefficients(predictions_data,coefficients_data,csv_filepath,json_filepath):
    ...
    combined_df=pd.merge(predictions_df,coefficients_df,on=['Lag','Prediction_DateTime'],how='left')
